...
const Modal : React.FC<ModalProps> = ({label , content , isOpen, close}) => {
    ...
    useEffect(() => {
        setShowModal(isOpen)
    }, [isOpen]) // any time isOpen is changed we rerender and setShowModal to isOpen value whether true or false
  ...
  return (
    ...
    <div className="relative w-[90%] md:w-[80%] lg:w-[700px] my-6 h-auto">
        <div className={`translate duration-600 h-full ${showModal ? 'translate-y-0 opacity-100' : 'translate-y-full opacity-10'}`}>
            ...
            <header className="flex items-center h-[60px] p-6 rounded-t justify-center relative border-b">
